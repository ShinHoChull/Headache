...
//                                        obj.isNull("addr")?"":obj.getString("addr")));
        ...
        this.binding.hospital.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if ( count == 0 ) {
                    ...
                    for ( int i = 0, j = hospitalDTOArrayList.size(); i < j ; i++) {
                        ...
                        if (row.getName().contains(binding.hospital.getText().toString())) {
        ...
        this.binding.listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                binding.hospital.setText(row.getName());
                hospital_sid = row.getSid();

                copyArrayList.clear();

                InputMethodManager imm = (InputMethodManager) getSystemService(INPUT_METHOD_SERVICE);
                imm.hideSoftInputFromWindow(binding.hospital.getWindowToken(), 0);
            }
        });
    }
    ...
    private void setBoyGirlCheck(int num) {
        if (num == 0) {
            ...
            this.binding.listLine.setVisibility(View.VISIBLE);
            ...
            this.binding.listLine.setVisibility(View.INVISIBLE);
            this.menuSetting("", "M");
        }
    }
    ...
    private boolean login() {
        if (this.binding.email.getText().toString().equals("")) {
            ...
            return false;
        } else if ( this.binding.hospital.getText().toString().equals("") ) {
            Toast.makeText(this, "내원병원을 입력해주세요.", Toast.LENGTH_SHORT).show();
            return false;
        }
        ...
        AndroidNetworking.post(this.urls.mainUrl + this.urls.getUrls.get("setLogin"))
                ...
                .addBodyParameter("hospital", this.binding.hospital.getText().toString())
                .addBodyParameter("hospital_sid", String.valueOf(this.hospital_sid))
            ...
            public void onResponse(JSONObject response) {
                ...
                try {
                    if (response.getString("rows").equals("Y")) {
                        ...
                        csp.put("joinId",binding.email.getText().toString() + "@" + binding.emailInput.getText().toString());
                        joinSuccess();
                    } else {
                        Toast.makeText(getApplicationContext() , response.getString("msg"), Toast.LENGTH_SHORT).show();
                    }
